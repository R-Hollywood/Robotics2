...
def rotate_pose(point, quat, theta, axis="x"):
    ...
    quat_t3d = [quat.w, quat.x, quat.y, quat.z]
...
def ik_baxter():
    ...
    point,orient=query_transformation('base','left_gripper')    
    point_out,quat_out =rotate_pose(point,orient,np.deg2rad(90),axis='z')
    pose_grip = generate_posemsg(point_out, quat_out)
    grip_joints=ik_service(srv, pose_grip)
    armcmd.move_to_joint_positions(grip_joints)
